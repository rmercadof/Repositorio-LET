...
```
```{r}
outliers <- boxplot(datos$PtjeLectura, plot=F)$out
datos_sout<- datos[-which(datos$PtjeLectura %in% outliers),]
```
...
modelo_lectura2 <- lm(PtjeLectura~ Autoestima + Convivencia + HabitosSaludables + FormacionCiudadana, data=datos_sout)
...
```{r}
#busqueda outliers
residuos <- rstandard(modelo_lectura2)

plot(modelo_lectura2$fitted.values, residuos, pch=19, xlab="Valores ajustados", ylab="Residuos estandarizados", ylim=c(-4,4))
abline(h=qnorm(0.975), col="blue", lwd=2); abline(h=-qnorm(0.975), col="blue", lwd=2)

```
```{r}
#puntos de palanca
hii <- hatvalues(modelo_lectura)
residuos2 <- rstandard(modelo_lectura)
p=12
linea <- 2*(p+1)/26944

plot(hii, residuos2, pch=19, xlab=expression(h[ii](leverage)),
     ylab="standardized residual")
abline(v=linea, col="red", lwd=2)
```
```{r}
# Load the package
library(ggstatsplot)
Q <- quantile(datos$PtjeLectura, probs=c(.25, .75), na.rm = FALSE)
iqr <- IQR(datos$PtjeLectura)
eliminated<- subset(datos, datos$PtjeLectura > (Q[1] - 1.5*iqr) & warpbreaks$breaks < (Q[2]+1.5*iqr))
...
```{r}
qqnorm(datos$PtjeLectura)
qqline(datos$PtjeLectura, col="red")
...
Al realizar el análisis de regresión fue claro que el modelo de regresión lineal más adecuado era el que incluía  como variables explicativas, todas las variables que habíamos considerado, es decir, el nivel socioeconómico, la convivencia escolar, los hábitos saludables, el autoestima y el criterio de formación ciudadana. Esto es positivo en el sentido que la base de datos se utilizó en su totalidad para realizar el modelo, pero también es cabe mencionar que el $R^2$ es muy bajo, es decir, el modelo no ajusta muy bien. 


# Referencias

En esta sección se deben presentar las referencias citadas a lo largo del informe utilizando de manera consistente un estilo de citación a elección (APA, ISO, Vancouver, MLA, etc.).
